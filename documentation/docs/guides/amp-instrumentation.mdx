...
The runnable [`dotnet-agent`](https://github.com/wso2/agent-manager/tree/main/samples/dotnet-agent) sample is the worked example of everything below. [Sample Agents](../tutorials/sample-agents.mdx) lists the other six samples and which instrumentation path each one takes.
...
What does vary is how far each framework has been verified. The sections below separate what is tested from what merely ships.
...
### Shipped but not yet verified

These instrumentations are installed and will attempt to emit spans, but have no coverage in the matrix test suite. Treat them as best-effort:

- Haystack
- OpenAI Agents SDK
- Agno
- LiteLLM (0.4.0 and later only)
- MCP

### Not instrumented

`strands-agents` has no instrumentation package in any AMP-instrumentation version. Agents built on it produce no automatic traces. Use the [manual instrumentation](#manual-instrumentation) path instead.
