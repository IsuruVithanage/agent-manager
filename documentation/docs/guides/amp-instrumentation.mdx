...
AMP collects spans from frameworks instrumented by the Traceloop SDK and from frameworks, such as Strands, that emit their own OpenTelemetry spans into AMP's tracer provider. The matrix below lists the framework versions tested with AMP instrumentation.
...
| Strands Agents | 1.55.1 | Agent, Chain, LLM, Tool | Tested with OpenAI and a local tool; Bedrock and MCP are not covered |
